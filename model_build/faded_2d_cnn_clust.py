...
if __name__ == "__main__":
    ...
    train_dataset = Simple2DPossSet(train_pklfiles, targets=False, transform=rflip)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4)

    validation_dataset = Simple2DPossSet(validation_pklfiles, targets=False, transform=rflip)
    ...
    # model = ResNet(BasicBlock, [3, 4, 6, 3], num_classes=10, n_chans=6).to(device)
    # model = ResNet(BasicBlock, [2, 2, 2, 2], num_classes=10, n_chans=6).to(device)
    # model = MyRes(block=BasicBlock, layers=[2, 2, 2, 2], num_classes=8, n_chans=6, outfunc=outfunc).to(device)

    optimizer = optim.Adam(model.parameters(), lr=learning_rate)
    criterion = ClusterLoss(try_n_clusters=try_n_clusters, out_device=device)
    ...
    for n in range(epochs):
        ...
        with torch.no_grad():
            for val_batchnum, val_coords_data in enumerate(validation_dataloader):
                ...
                batch_val_loss = criterion(val_outputs, is_val=True)
                total_val_loss += batch_val_loss.item()

        avg_val_loss = total_val_loss / (val_batchnum + 1)
    ...
    torch.save(model, 'prod_faded_cnn.mdl')